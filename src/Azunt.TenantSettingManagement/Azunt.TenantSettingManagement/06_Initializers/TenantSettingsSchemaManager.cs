...
CREATE TABLE [dbo].[TenantSettings](
    ...
    [UpdatedAt]  DATETIMEOFFSET(7)  NOT NULL 
        CONSTRAINT DF_TenantSettings_UpdatedAt DEFAULT ({UtcNowSql}),
...
WHERE c.object_id = OBJECT_ID('dbo.TenantSettings') 
  AND c.name = 'UpdatedAt';", connection))
            {
...
BEGIN TRY
    ...
    BEGIN
        DECLARE @sqlDrop nvarchar(4000) =
            N'ALTER TABLE dbo.TenantSettings DROP CONSTRAINT ' + QUOTENAME(@dfName) + N';';
        EXEC (@sqlDrop);
    END
